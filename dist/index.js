...
function deploy(content, id, token, guild, debug) {
    ...
    if (isDebugging)
        npmlog_1["default"].info("Debug", "Debug output is enabled. Disable it by removing the -d option or not passing the debug parameter.");
    var endpoint = "https://discord.com/api/v10/applications/".concat(id, "/commands");
    if (guild !== undefined && guild !== null) {
        endpoint = "https://discord.com/api/v10/applications/".concat(id, "/guilds/").concat(guild, "/commands");
    }
    var array;
    if (content.commands !== undefined) {
        ...
        if (Array.isArray(content)) {
            ...
            throw new ParseError("'commands' needs to be an array of commands or an object with an 'commands' property.", ["root"]);
        }
    }
            ...
            command.subcommands.forEach(function (subcommand) {
                ...
                if (subcommand.group !== undefined) {
                    if (groups_1.has(subcommand.group)) {
                        ...
                        if (def === undefined) {
                            throw new ParseError("Subcommand group ".concat(subcommand.group, " is not defined"), [command.name, subcommand.name]);
                        }
                        var group = {
...
function sendData(endpoint, token, object) {
    ...
    if (isDebugging) {
        ...
        (0, fs_1.writeFile)("commandcord-debugging-" + random_1 + ".json", JSON.stringify(object), function () {
            npmlog_1["default"].info("Debug", "Saved request body to commandcord-debugging-" + random_1 + ".json");
        });
    }
    (0, undici_1.fetch)(endpoint, {
        ...
        .then(function (res) {
        if (res.status !== 200) {
            npmlog_1["default"].error("API", "The request did not complete successfully. %s", res.statusText);
        }
        else {
            npmlog_1["default"].info("API", "Success! Your commands were deployed and should appear in Discord. Note, this may take a while.");
        }
    })["catch"](function (err) {
...
function doCommandChecks(command) {
    ...
    if (command.subcommands === undefined &&
        ...
        npmlog_1["default"].warn("Parser", "Unnecessary property 'subcommandGroups'.", tracePath([command.name]));
}
...
var ParseError = /** @class */ (function (_super) {
    ...
    function ParseError(message, path) {
        return _super.call(this, message + "\nLocated at: ".concat(tracePath(path))) || this;
    }
    return ParseError;
}(Error));
